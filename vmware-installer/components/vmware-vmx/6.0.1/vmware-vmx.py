...
class VMX(Installer):
   def InitializeQuestions(self, old, new, upgrade):
      ...
      self.inVM = False
      self.checkVMBytes = self.GetFileText('extra/checkvm')
      ...
      self.checkvmBin.write_bytes(self.checkVMBytes)
      self.checkvmBin.chmod(0755)
            ...
            if not self.forceInstallInVM: # Only check if they haven't been overridden
               if not toolsVersion:
                   msg = 'You are installing inside a virtual machine. It is' \
                         ...
                         ' this product, the two products may not work together.' % minVersion
                   log.Warn(msg)
   ...
   def InitializeInstall(self, old, new, upgrade):
      if self._checkXenPresence():
         log.Warn(u'This system is running a Xen kernel. You cannot run VMs under the Xen kernel.')

      if path('/dev/kvm').exists():
         log.Warn(u'This system has KVM enabled. You cannot run VMs with KVM enabled.')
      ...
      self.SetPermission(DEST/'lib/wrapper-gtk24.sh', BINARY)
      self.SetPermission(DEST/'lib/libgksu2.so.0/gksu-run-helper', BINARY)
   ...
   def PreUninstall(self, old, new, upgrade):
      ...
      if ENV.get('VMWARE_SKIP_SERVICES'):
         ...
         log.Error(u'Unable to stop VMware services')
   ...
   def PostInstall(self, old, new, upgrade):
      ...
      # be manually set up
      if not INITDIR:
         self.UserMessage(u'No rc*.d style init script directories were given to the installer. '
                          u'You must manually add the necessary links to ensure that the vmware '
                          u'service at %s is automatically started and stopped on startup and shutdown.' % str(INITSCRIPTDIR/'vmware'))
   ...
   def _checkXenPresence(self):
      ...
      return xenTest.exists() and len(xenTest.bytes())
