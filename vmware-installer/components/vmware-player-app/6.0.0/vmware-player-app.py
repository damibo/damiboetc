...
class PlayerApp(Installer):
   ...
   def InitializeInstall(self, old, new, upgrade):
      ...
      self.SetPermission(SYSCONFDIR/'init.d/vmamqpd', BINARY)
   ...
   def _killVMwareProcesses(self, upgrade):
      self.RunCommand('killall',
                      ...
                      'vmplayer-daemon', 'vmware-enter-serial',
   ...
   def PreUninstall(self, old, new, upgrade):
      ...
      # Stop our init scripts for uninstallation
      for scriptName in ['vmamqpd']:
      ...
      inits.DeconfigureService('vmamqpd')
   ...
   def PostInstall(self, old, new, upgrade):
      ...
      inits = self.LoadInclude('initscript')
      inits.ConfigureService('vmamqpd',
                             'This service starts and stops vmamqpd',
                             '$network', # Start
                             '$network', # Stop
                             '',
                             '',
      ...
      # init scripts
      for scriptName in ['vmware', 'vmamqpd']:
         script = INITSCRIPTDIR/scriptName
         if INITSCRIPTDIR and script.exists():
            self.RunCommand(script, 'stop', ignoreErrors=True)
   ...
   def AddUninstallLinks(self, suffix=None):
      ...
      for link in links:
         ...
         bin = LIBDIR/'vmware-installer/2.1.0'
         bin.perm = BINARY
